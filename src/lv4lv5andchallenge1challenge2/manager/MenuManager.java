...
public class MenuManager {
    ...
    public Food[] getFoodOfCategory(int categoryIndex) {
        ...
        return Arrays.stream(allFoods).filter(food -> foodCountMap.get(food) == categoryIndex).toArray(Food[]::new);
    }
    ...
    public Optional<Food> selectFood(int categoryIndex, int index) {
        ...
        String category = Arrays.stream(categoryArray).filter(item -> categoryIndexMap.get(item).equals(categoryIndex)).findFirst().get();

        Food[] foods = foodCountMap.keySet().toArray(new Food[0]);
        Optional<Food> food = Arrays.stream(foods).filter(item -> Objects.equals(item.getCategory(), category) && item.getIndex() == index).findFirst();
